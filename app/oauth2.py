from fastapi import Depends , status , HTTPException
from jose import JWTError,jwt
from datetime import date, datetime , time, timedelta
from fastapi.security import OAuth2PasswordBearer
from .config import settings
oauth2_shceme = OAuth2PasswordBearer(tokenUrl="login")

from app import schemas , database,models
from sqlalchemy.orm import Session


SECRET_KEY = settings.secret_key

ALGORITHM = settings.algorithem
ACCESS_TOKEN_EXPIRE_MINUTES = settings.access_token_expire_minutes

def create_acsses_token(data:dict):

    to_encode = data.copy()
    expire = datetime.utcnow() + timedelta(minutes=ACCESS_TOKEN_EXPIRE_MINUTES)
    to_encode.update({"exp":expire})
    encoded_jwt = jwt.encode(to_encode , SECRET_KEY , algorithm=ALGORITHM)

    return encoded_jwt



def verfiy_access_token(token:str , creddentials_exception)  :
    try:
            payload = jwt.decode(token , SECRET_KEY , algorithms=[ALGORITHM])

            id:str = payload.get("user_id")

            if id is None:
                raise creddentials_exception

            token_data = schemas.TokenData(id = id)    
    except JWTError :
        raise creddentials_exception
    return token_data    

    
def get_curren_user(token:str = Depends(oauth2_shceme) , db:Session  = Depends(database.get_db)):

    credetials_exception = HTTPException(status_code=status.HTTP_401_UNAUTHORIZED , detail="Could not validate credentials" , headers={"WWW-Authenticate":"Bearer"})
    token  = verfiy_access_token(token  , credetials_exception)
    user  =  db.query(models.User).filter(models.User.id==token.id).first()
    return user